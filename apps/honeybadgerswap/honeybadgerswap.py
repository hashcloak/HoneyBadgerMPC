...
async def eth_to_token(
    ...
    fee = eth_to_exchange / 500
    k = init_eth_pool * init_token_pool
    new_eth_pool = init_eth_pool + eth_to_exchange
    new_token_pool = k / (new_eth_pool - fee)
...
async def token_to_eth(
    ...
    fee = tokens_to_exchange / 500
    k = init_eth_pool * init_token_pool
    new_token_pool = init_token_pool + tokens_to_exchange
    new_eth_pool = k / (new_token_pool - fee)
...
def create_secret_share(ctx, x):
    return FixedPoint(ctx, ctx.Share(x * 2 ** 32) + ctx.prepoc.get_zero(ctx))


async def prog(ctx):
    init_eth_pool = create_secret_share(ctx, 10)
    init_token_pool = create_secret_share(ctx, 500)
    eth_to_exchange = create_secret_share(ctx, 1)
